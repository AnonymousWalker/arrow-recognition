...
def process_arrows(window, lock=None):    
    ...
    captured_image = capture_screenshot_app_window(window, config.ARROW_REGION)
    global debug_img
...
def send_key_input(window, arrows):
    ...
    for arr in arrows:
        ...
        time.sleep(config.KEY_TYPING_SLEEP)
...
def key_listener(e, window):
    ...
    if e.event_type == keyboard.KEY_DOWN:
        ...
        if e.name == 'page up':
            speed += config.ADJUST_SPEED_AMOUNT
        ...
        if e.name == 'f11':
            ...
            print(f"Level: {config.CURRENT_LEVEL}")
        if e.name == 'f10': 
            config.CURRENT_LEVEL -= 1
            print(f"Level: {config.CURRENT_LEVEL}")
        # if e.name == 'f3':
...
def watch_to_hit_perfect(window, head_img, track_area):
    ...
    try:
        remaining_time = (config.PERFECT_POS_X - head_X) / speed # distance / speed
...
def wait_keys_appear(window, track_area):
    while True:
        ...
        if headX >= config.PERFECT_POS_X:
            if config.CURRENT_LEVEL < 6:
                time_to_process = config.PIXELS_TO_PROCESS / speed # for low level (1-5)
                time.sleep(time_to_process)
            elif speed < 97:
                time_to_process = config.PIXELS_TO_PROCESS / speed / 2
                time.sleep(time_to_process)
            elif speed < 110:
                time.sleep(0.01)
            return
